...
object dsl {
  ...
  class AsyncValidatorAsyncOps[M[_], T](val v: AsyncValidator[M, T]) extends AnyVal {

    def compose(v2: AsyncValidator[M, T])(implicit app: App[M]): AsyncValidator[M, T] =
    ...
    def composeSuper[U >: T](v2: AsyncValidator[M, U])(implicit app: App[M]): AsyncValidator[M, T] =
    ...
    def composeDerived(implicit dav: DerivedAsyncValidator[M, T], app: App[M]): AsyncValidator[M, T] =
      compose(dav.av)
